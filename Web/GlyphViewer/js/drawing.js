		...
		for (const contour of shape.contours) {
			...
			for (let ni = 0; ni < n; ni++) {
				...
				if (node.type === 'curve') {
					...
					// First BCP of pair → connects backward to on-curve
					if (prev.type === 'on') {
					...
					// Second BCP of pair → connects forward to on-curve
					if (next.type === 'on') {
					...
					// No line between two adjacent cubic BCPs
					...
					// Quadratic off-curve: connect to both adjacent on-curves
