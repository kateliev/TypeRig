...
#NOTE: Sort anchor order across masters - by reference master, alphabetically or custom list
...
# - Init --------------------------------
app_name, app_version = 'TR | Sort Anchors', '1.1'
...
class dlg_sort_anchors(QtGui.QDialog):
    def __init__(self):
        ...
        self.rad_mode_custom = QtGui.QRadioButton('Custom list')
        ...
        layout_mode.addWidget(self.rad_mode_custom)
        ...
        # - Custom list input
        self.lbl_custom_list = QtGui.QLabel('Custom anchor list (one per line):')
        self.txt_custom_list = QtGui.QTextEdit()
        self.txt_custom_list.setMaximumHeight(100)
        ...
        self.chk_report_orphans = QtGui.QCheckBox('Report orphaned anchors (not in list)')
        self.chk_report_orphans.setChecked(True)
        grp_options = QtGui.QGroupBox('Options')
        layout_options = QtGui.QVBoxLayout()
        layout_options.addWidget(self.chk_verbose)
        layout_options.addWidget(self.chk_report_orphans)
        ...
        self.rad_mode_custom.toggled.connect(self.update_mode_state)
        
        # - Main layout
        layout_main = QtGui.QVBoxLayout()
        layout_main.addWidget(grp_mode)
        layout_main.addLayout(layout_master)
        layout_main.addWidget(self.lbl_custom_list)
        layout_main.addWidget(self.txt_custom_list)
    ...
    def update_mode_state(self):
        ...
        is_custom = self.rad_mode_custom.isChecked()
        self.lbl_master.setEnabled(is_reference)
        self.cmb_master.setEnabled(is_reference)
        self.lbl_custom_list.setEnabled(is_custom)
        self.txt_custom_list.setEnabled(is_custom)
    def get_glyphs(self):
        ...
        if self.rad_scope_selected.isChecked():
            ...
            return getProcessGlyphs(mode=3)
    ...
    def get_custom_list(self):
        '''Parse custom anchor list from text input'''
        text = self.txt_custom_list.toPlainText()
        if not text:
            return []
    ...
    def action_execute(self):
        ...
        report_orphans = self.chk_report_orphans.isChecked()
        is_reference_mode = self.rad_mode_reference.isChecked()
        is_custom_mode = self.rad_mode_custom.isChecked()
        ...
        for glyph_obj in glyphs:
            ...
            if is_reference_mode:
                ...
                # Get custom anchor list
                ordered_names = self.get_custom_list()
                if not ordered_names:
                    output(1, app_name, 'No custom anchor list provided for glyph: {}'.format(glyph_name))
                    glyphs_skipped += 1
                    continue

                # Collect all anchors in this glyph across all masters for orphaned anchor reporting
                all_glyph_anchors = set()
                for master in all_masters:
                    if glyph_obj.layer(master) is not None:
                        for a in glyph_obj.anchors(master):
                            all_glyph_anchors.add(a.name)

                # Report orphaned anchors if requested
                if report_orphans:
                    orphaned_anchors = [name for name in all_glyph_anchors if name not in ordered_names]
                    if orphaned_anchors:
                        output(1, app_name, 'Orphaned anchors in glyph [{}] (not in list): {}'.format(
                            glyph_name, ', '.join(orphaned_anchors)))

                # Apply custom order to all masters
                for master in all_masters:
                    self.reorder_anchors(glyph_obj, master, ordered_names)
            else:
        ...
        if is_custom_mode:
            mode_label = 'custom list'
        elif is_reference_mode:
            mode_label = 'reference [{}]'.format(reference_master)
        else:
            mode_label = 'alphabetical'
