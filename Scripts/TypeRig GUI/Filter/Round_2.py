	...
	def __processGlyph(self, glyph, indices, preset):
		...
		for layer in wLayers:
			if layer in preset.keys() and len(indices):
				# - Init
				nodes_at_shapes = {}
				selection = glyph.selectedAtShapes(layer=layer, index=False, deep=True)

				# - Build note to shape reference
				for shape, contour, node in selection:
					nodes_at_shapes.setdefault(shape,[]).append(node)
				
				# - Build filter if not present
				if not len(glyph.containers(layer)):
					for shape in nodes_at_shapes.keys():
						new_container = fl6.flShape()
						new_container.shapeData.name = shape.shapeData.name
						shape.shapeData.name = shape.shapeData.name + '.old'
						#new_container.include(glyph.shapes(layer), glyph.layer(layer))
						new_container.include(shape, shape)
				...
				process_nodes = [pNode(node) for node in nodes_at_shapes.values()]
